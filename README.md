...
### Обновление файла nextgisweb/__init__.py
Этот файл содержит функцию pkginfo(), которая возвращает словарь компонентов:

    {'components': 
        {
          'auth': 'nextgisweb.auth',
          'core': 'nextgisweb.core',
          'feature_description': 'nextgisweb.feature_description',
          'feature_layer': 'nextgisweb.feature_layer',
            ...
         }
    }
    
следует добавить в эту функцию описание вновь созданного компонента.

Функция amd_packages() из файла nextgisweb/__init__.py также содержит список компонент. В нее также следует добавить описание вновь созданного компонента.

### Создание пакета, содержащего программный код компонента
Обычно компонент состоит из следующих файлов, в которых реализован программный код:

    nextgisweb/component_name/__init__.py 
    nextgisweb/component_name/model.py 
    nextgisweb/component_name/view.py
    nextgisweb/component_name/amd/ngw-component_name/ComponentWidget.js 
    nextgisweb/component_name/amd/ngw-component_name/template/ComponentWidget.html 
    
Первые три файла \__init__.py, model.py и view.py --- файлы, код которых обрабатыватся фреймворком Pyramid. ComponentWidget.js и ComponentWidget.html --- файлы, содержащие
код виджета, который основан на dojo. Ниже идет краткое описание каждого файла.

model.py -- обычный файл с описанием модели, его назначение и принципы работы хорошо описаны в документации Pyramid
view.py -- файл представлений Pyramid, в этом файле конфигурируются пути (view_config) к  представлениям (см. функцию setup_pyramid).
