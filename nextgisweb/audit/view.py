...
def journal_browse(request):
    ...
    if date_first:
        kwargs["order"] = "asc"
        date_first = datetime.strptime(date_first, DATE_FORMAT) + timedelta(
            milliseconds=1
        )
...
def setup_pyramid(comp, config):
    ...
    if comp.audit_enabled:
        ...
        if comp.audit_es_enabled:
            config.add_route(
                "audit.control_panel.journal.browse", "/control-panel/journal/"
            ).add_view(journal_browse, renderer="nextgisweb:audit/template/browse.mako")

            config.add_route(
                "audit.control_panel.journal.show", "/control-panel/journal/{id}"
