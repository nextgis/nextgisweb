...
tab_group_user = sa.Table(
    ...
    sa.Column(
        "user_id", sa.Integer, sa.ForeignKey("auth_user.principal_id"), primary_key=True
    ),
)
...
class User(Principal):
    ...
    def is_administrator(self):
        ...
        return any(
            [
                user
                for user in self._admins.members
    ...
    def deserialize(self, data):
        ...
        if "member_of" in data:
            self.member_of = [
                Group.filter_by(id=gid).one() for gid in data["member_of"]
            ]
...
class Group(Principal):
    ...
    def deserialize(self, data):
        ...
        if "members" in data:
            self.members = [User.filter_by(id=uid).one() for uid in data["members"]]
