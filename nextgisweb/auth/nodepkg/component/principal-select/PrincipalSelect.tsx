...
export function PrincipalSelect({
  ...
  const optionRender = ({ label, value }: { label: string; value: number }) => {
    return (
      <Space>
                ...
                .filter((itm) => {
                  if (itm.system) {
                    if (Array.isArray(systemUsers)) {
                      return systemUsers.includes(itm.keyname);
                    }
                    return !!systemUsers;
                  }
                  return true;
      ...
      if (loadGroups) {
        promises.push(
          route("auth.group.collection")
            ...
            .then((data) => data.map((data) => ({ ...data, _user: false })))
        );
      }
