...
class OAuthHelper(object):
    def __init__(self, options):
        ...
        if "server.authorization_header" in options:
            self.server_headers["Authorization"] = options[
    ...
    def grant_type_password(self, username, password):
        ...
        return self._token_request(
            "password", dict(username=username, password=password)
        )
    ...
    def grant_type_refresh_token(self, refresh_token, access_token):
        try:
            return self._token_request(
                "refresh_token",
                dict(refresh_token=refresh_token, access_token=access_token),
            )
    ...
    def query_introspection(self, access_token):
        if len(access_token) > MAX_TOKEN_LENGTH:
            token_id = "sha512:" + sha512(six.ensure_binary(access_token)).hexdigest()
        else:
    ...
    def access_token_to_user(self, access_token, merge_user=None):
        ...
        with DBSession.no_autoflush:
            ...
            if (
                ...
                and user.oauth_tstamp + self.options["profile.sync_timedelta"]
                > datetime.utcnow()
            ):
    ...
    def _server_request(self, endpoint, params):
        ...
        response = getattr(requests, method.lower())(
            url, params, headers=self.server_headers
        )
    ...
    def _update_user(self, user, token):
        ...
        if mof_attr is not None:
            ...
            grp_map = dict(map(lambda i: i.split(":"), opts["member_of.map"]))

            grp_keyname = list(map(lambda i: grp_map.get(i, i), mof))
    ...
    option_annotations = OptionAnnotations(
        (
            ...
            Option(
                "client.secret", default=None, secure=True, doc="OAuth client secret"
            ),
            Option(
                "server.password",
                bool,
            ...
            Option(
                "server.introspection_endpoint",
                default=None,
                doc="OAuth token introspection endpoint URL.",
            ),
            Option(
                "server.introspection_method",
                default="POST",
...
OAuthGrantResponse = namedtuple(
    "OAuthGrantResponse", ["access_token", "refresh_token", "expires"]
)
...
def _fallback_value(*args):
    for a in args:
        if not (a is None or (isinstance(a, six.string_types) and a.strip() == "")):
            return a
