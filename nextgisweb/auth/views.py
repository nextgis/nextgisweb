...
from .models import Group, Principal, User
    ...
    if "error" in request.params:
        ...
        if data["merge"] == "1" and request.user.keyname != "guest":
            user = oaserver.access_token_to_user(
                tresp.access_token, merge_user=request.user
            )
        else:
...
def _login_url(request):
    ...
    if (
        oauth_opts["enabled"]
        and oauth_opts["default"]
        and not oauth_opts["server.password"]
    ):
...
def setup_pyramid(comp, config):
    ...
    config.add_route("auth.login", "/login").add_view(
        login, renderer="nextgisweb:auth/template/login.mako"
    )
    ...
    config.add_route("auth.settings", "/settings").add_view(
        settings, renderer="nextgisweb:auth/template/settings.mako"
    )
    ...
    config.add_route("auth.principal_dump", "/auth/principal/dump").add_view(
        principal_dump, renderer="json"
    )
    ...
    class AuthGroupWidget(ObjectWidget):
        ...
        def validate(self):
            ...
            if self.operation in ("create", "edit"):
                query = Group.filter_by(keyname=self.data.get("keyname"))
        ...
        def widget_params(self):
            ...
            if self.obj:
                ...
                result["users"] = [
                    dict(
                        value=u.id, label=u.display_name, selected=u in self.obj.members
                    )
    ...
    class GroupController(ModelController):
        def create_context(self, request):
            ...
            return dict(
                template=dict(subtitle=_("Create new group"), dynmenu=Group.__dynmenu__)
            )

        def edit_context(self, request):
            check_permission(request)
            obj = Group.filter_by(**request.matchdict).filter_by(system=False).one()

            return dict(obj=obj, template=dict(obj=obj))
    ...
    class AuthUserWidget(ObjectWidget):
                ...
                if disabled or "member_of" in self.data:
                    ...
                    if not disabled and admins.id in self.data["member_of"]:
                        ...
                        [
                            ...
                            and user.principal_id != self.obj.principal_id
                        ]
                    ):
                        result = False
                        self.error.append(
                            dict(
                                message=self.request.localizer.translate(
                                    _(
                                        "You can't disable current administrator. At least one enabled administrator is required."
                                    )
                                )
                            )
                        )  # NOQA
        ...
        def widget_params(self):
            ...
            if self.obj:
                ...
                result["groups"] = [
                    dict(value=g.id, label=g.display_name) for g in Group.query()
                ]
    ...
    class AuthUserModelController(ModelController):
        def create_context(self, request):
            check_permission(request)
            return dict(
                template=dict(subtitle=_("Create new user"), dynmenu=User.__dynmenu__)
            )

        def edit_context(self, request):
            check_permission(request)
            obj = User.filter_by(**request.matchdict).filter_by(system=False).one()
    ...
    config.add_route("auth.user.browse", "/auth/user/").add_view(
        user_browse, renderer="nextgisweb:auth/template/user_browse.mako"
    )
    ...
    config.add_route("auth.group.browse", "/auth/group/").add_view(
        group_browse, renderer="nextgisweb:auth/template/group_browse.mako"
    )
            ...
            if "obj" in kwargs and isinstance(kwargs.obj, User):
                yield dm.Link(
                    ...
                    lambda kwargs: kwargs.request.route_url(
                        "auth.user.edit", id=kwargs.obj.id
                    ),
                )
        ...
        def build(self, kwargs):
            ...
            if "obj" in kwargs and isinstance(kwargs.obj, Group):
                yield dm.Link(
                    ...
                    _("Edit"),
                    lambda kwargs: kwargs.request.route_url(
                        "auth.group.edit", id=kwargs.obj.id
                    ),
                )
