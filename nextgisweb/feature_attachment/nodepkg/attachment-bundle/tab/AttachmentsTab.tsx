        ...
        <div className="row" title={gettext("Object: ") + label}>
          ...
          <div>
            <Button
              shape="circle"
              type="text"
              onClick={() => identifyFeature(item.featureId, item.layerId)}
...
const makeAttachmentsList = (
  ...
  return (
    <div className="attachments-list" key={layerItemView.layerLabel}>
      <Divider orientation="horizontal">{layerItemView.layerLabel}</Divider>
      <List
        size="small"
        bordered
        dataSource={data}
    ...
    if (layerInfo.type === "layer") {
      ...
      featuresInfo.forEach((f: FeatureItem) => {
        if (f.extensions && f.extensions.attachment) {
          ...
          const attachmentsView: FeatureAttachmentView[] = attachmentsInfo.map(
            (a: FeatureAttachment) => ({
              featureId: f.id,
              attachment: a,
              label: f.label,
