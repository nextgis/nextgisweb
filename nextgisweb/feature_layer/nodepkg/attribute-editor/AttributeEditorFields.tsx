...
export interface AttributeEditorFieldsProps {
  store: AttributeEditorStore;
  size?: SizeType;
  onChange?: (value: NgwAttributeValue | null) => void;
}

export const AttributeEditorFields = observer(
  ({ store, size = "middle", onChange }: AttributeEditorFieldsProps) => {
    const {
      fields,
      saving,
      isReady,
    ...
    return (
      ...
      >
        {!formFields.length && (
          <Typography.Paragraph>{msgNoAttrs}</Typography.Paragraph>
        )}
      </FieldsForm>
    );
  }
...
AttributeEditorFields.displayName = "AttributeEditorFields";
