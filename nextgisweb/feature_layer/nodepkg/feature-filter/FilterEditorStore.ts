...
export class FilterEditorStore {
    ...
    if (tab === "constructor") {
      if (this.jsonValue) {
        try {
          ...
          console.error(
            gettext("Failed to parse JSON, falling back to JSON tab."),
            error
          );
          this.isValid = false;
          return;
        ...
        this.filterState = {
          rootGroup: parseFilterExpression([], this.generateTransientId),
        };
        this.isValid = true;
      }
    }
  ...
  private _movePendingItemToGroupByPosition(
    ...
    group.childrenOrder = group.childrenOrder.filter(
      (child) => !(child.type === filterType && child.id === pendingItem.id)
    );

    const anchorIndex =
      targetItemId === undefined
        ? -1
        : group.childrenOrder.findIndex((child) => child.id === targetItemId);
    const insertIndex =
      anchorIndex === -1
    ...
    const items = filterType === "condition" ? group.conditions : group.groups;
  ...
  private _findAndRemoveCondition(
    ...
    if (index !== -1) {
      ...
      group.childrenOrder = group.childrenOrder.filter(
        (child) => !(child.type === "condition" && child.id === removed.id)
      );
      return removed;
    }
    for (const childGroup of group.groups) {
  ...
  private convertToFilterExpression(group: FilterGroup): FilterExpression {
    ...
    for (const child of group.childrenOrder) {
      if (child.type === "condition") {
        const condition = group.conditions.find((c) => c.id === child.id);
        if (condition) {
  ...
  private validateCurrentState() {
    try {
      ...
      if (this.activeTab === "json" && this.jsonValue) {
        ...
        expression = this.convertToFilterExpression(this.filterState.rootGroup);
        validateFilterExpression(expression, this.fields);
      }
      this.validJsonValue =
        expression.length === 0 ? undefined : stringifyExpresion(expression);
      this.isValid = true;
      this.validationError = undefined;
    } catch (error) {
      this.isValid = false;
      this.validationError =
        error instanceof Error ? error.message : gettext("Validation failed");
  ...
  private _insertChild<T extends FilterCondition | FilterGroup>(
    ...
    const existingOrderIndex = targetGroup.childrenOrder.findIndex(
      (child) => child.type === descriptor.type && child.id === descriptor.id
    );
