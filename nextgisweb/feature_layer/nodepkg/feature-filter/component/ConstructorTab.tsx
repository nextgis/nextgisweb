...
const getInsertionGroupPosition = (
  ...
  const targetGroupOverItems = allOverItems.filter((i) => {
    return (
      childrenOrderMap.has(i.id as number) && targetGroup.id !== sourceItemId
    );
  });
  ...
  if (targetGroupOverItems.length === 0) {
    ...
    targetGroupOverItems.forEach((i) => {
      const id = i.id;
      const index = targetGroup.childrenOrder.findIndex((i) => i.id === id);
      if (index !== -1) {
    ...
    <div className="filter-constructor">
      <div className="filter-root">
        ...
        >
          ...
          />
          <DragOverlay>
            {activeId ? <div className="drag-overlay-ghost"></div> : null}
          </DragOverlay>
        </DndContext>
      </div>
    </div>
