...
const GeometryEditor = observer(
  ({ store: storeProp }: GeometryEditorStoreProps) => {
    ...
    const layerOptions = useMemo<LayerOptions>(() => {
      return {
        style: (feature) => {
          const isHighlighted = String(feature.getId()) === String(featureId);
          if (isHighlighted) {
            return undefined;
          }
    ...
    return (
      ...
      >
        {store._parentStore?.resourceId && (
          <>
            {initExtent && (
              <ZoomToResourceBtn resourceId={store._parentStore.resourceId} />
            )}
            <NGWLayer
        ...
        <GeometryEditorControlPanel store={store} position="top-right" />
      </PreviewMap>
    );
  }
);
