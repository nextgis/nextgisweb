...
from typing import Any, Dict, List, Literal, NamedTuple, Optional, Tuple, Type, Union
...
class FVersioningExtensionMixin:
    fversioning_registry: Dict[str, Type[FVersioningExtensionMixin]] = dict()

    # Class attributes, descendants must define them
    fversioning_metadata_version: int
    fversioning_extension: str
    fversioning_columns: List[str]
    ...
    def __setup_metadata(cls):
        ...
        # raise ValueError(str(data_columns))
        cls.fversioning_cols: Tuple[str] = tuple(c.name for c in data_columns)
