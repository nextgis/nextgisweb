...
def auto_description(cls: S) -> S:
    ...
    if operation == "create":
        description = (
            "The {0} does not exist in the initial version and exists in the "
            "target version, thus it has been created. This can be the result "
            "of one (just create) or consequent operations such as create and "
            "update. In case of multiple operations, the version ID "
        ...
        description = (
            ...
            "target version, thus it has been deleted. In case of multiple "
            "consequent operations, the version ID corresponds to the last one."
        )
        if fscope != "feature":
            description += "\n\n" + (
