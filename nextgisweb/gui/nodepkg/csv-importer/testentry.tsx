...
const DEFAULT_COLUMNS: TargetColumn[] = [
  ...
  },
  {
    key: "description",
    label: "Description",
    aliases: ["Description", "description", "desc", "Desc"],
  },
  {
    key: "date",
    label: "Date",
    aliases: ["Date", "date", "created_at", "createdAt"],
  },
  {
    key: "amount",
    label: "Amount",
    aliases: ["Amount", "amount", "price", "Price"],
  },
];

export default function CsvImporterTestEntry() {
  const [columns, setColumns] = useState<TargetColumn[]>(DEFAULT_COLUMNS);
  const [aliasesRaw, setAliasesRaw] = useState<string[]>(
    DEFAULT_COLUMNS.map((c) => c.aliases.join(", "))
  );
  const [rows, setRows] = useState<CsvImporterRow[] | undefined>(undefined);
  ...
  const handlePaste = async () => {
    try {
      ...
      if (
        ...
        setColumns(parsed);
        setAliasesRaw(parsed.map((c: TargetColumn) => c.aliases.join(", ")));
      } else {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "flex-start" }}>
        <Button type="primary" loading={isLoading} onClick={openImporter}>
          {"Open CsvImporterModal"}
        </Button>
      </div>
