...
> implements ComplexTreeEnvironment<I> {
  ...
  get selected(): I | null {
    const state = this.target.viewState[this.treeId] as IndividualTreeViewState;
...
function Container(props: {
  ...
  return (
    <div
      ref={ref}
      className={classNames("ngw-gui-focus-table-complex-tree", className)}
...
>({
  ...
  const onSelectItems = useCallback(
    (indexes: TreeItemIndex[]) => {
      onSelect?.(
        indexes.length > 0 ? provider.indexer.lookup(indexes[0]) || null : null
      );
    },
    [onSelect, provider]
  );
