...
export function ModelBrowse<Data extends ModalBrowseData = ModalBrowseData>({
  ...
  const { data, isLoading, refresh } = useRouteGet<Data[]>({
  ...
  useEffect(() => {
    if (reloadToken !== prevReloadTokenRef.current) {
      ...
      if (deleteError.length) {
        modal.confirm({
          ...
          content: (
            <>
              <p>
                {gettext("Failed to delete items:")} {deleteError.join(", ")}
              </p>
            </>
          ),
        });
    ...
    tableColumns.push({
      ...
      render: (_, record) => (
        <div style={{ whiteSpace: "nowrap" }}>
          <Tooltip title={!readonly ? gettext("Edit") : gettext("View")}>
            <Button
              type="text"
              shape="circle"
