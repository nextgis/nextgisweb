...
Struct = defstruct("Struct", [("e", Decimal)])
Nested = defstruct("Nested", [("s", Struct)])

m_struct = Struct(Decimal("2.71"))
m_nested = Nested(m_struct)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(v_date, f'"{v_date.isoformat()}"', v_date.isoformat(), id="date"),
        pytest.param(v_time, f'"{v_time.isoformat()}"', v_time.isoformat(), id="time"),
        pytest.param(
            v_datetime,
            f'"{v_datetime.isoformat()}"',
            v_datetime.isoformat(),
            id="datetime",
        ),
        pytest.param(v_uuid, f'"{v_uuid}"', str(v_uuid), id="uuid"),
        pytest.param(
            [OrderedDict(a=0), dict(b=1)],
            '[{"a":0},{"b":1}]',
            [dict(a=0), dict(b=1)],
            id="complex",
        ),
