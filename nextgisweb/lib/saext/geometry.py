...
from nextgisweb.lib.geometry import Geometry as NGWGeometry
...
class Geometry(UserDefinedType):
    ...
    def bind_processor(self, dialect):
        ...
        def process(value):
            if value is None:
                return None
            if isinstance(value, NGWGeometry):
                geom_srid = value.srid if value.srid is not None else srid
                return f"SRID={geom_srid};{value.wkt}"
