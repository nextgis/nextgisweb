...
def dumper_factory(
    resource,
    dumper=feature_layer_api.Dumper,
    ...
    return dumper(resource, params)


def feature_to_ogc(dumper: feature_layer_api.Dumper, feature: Feature):
    data = dumper(feature)
    return dict(
        type="Feature",
        id=feature.id,
...
def loader_factory(
    resource,
    loader=feature_layer_api.Loader,
    params=feature_layer_api.LoaderParams(
        geom_format="geojson",
        dt_format="iso",
        srs=4326,
    ),
):
    return loader(resource, params)


def feature_from_ogc(loader: feature_layer_api.Loader, feature: Feature, data):
    data = data.copy()
...
def create(resource, request) -> JSONType:
    ...
    for c in resource.collections:
        if c.keyname == collection_id:
            ...
            with c.resource.transaction(request):
