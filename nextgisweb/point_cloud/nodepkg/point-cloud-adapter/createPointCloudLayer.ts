...
function computeNodeBudgets(
  ...
  const nodeWeights = selectedNodes.map((entry) => {
    return Math.max(
      1,
      entry.node.pointCount * viewportFraction(entry.bounds, viewExtent)
    );
  });
  const totalPointCount = selectedNodes.reduce(
    (sum, _entry, index) => sum + nodeWeights[index],
    0
  );
  ...
  return selectedNodes.map((entry, index) => {
    ...
    const proportional = totalPointCount
      ? Math.round((pointBudget * nodeWeights[index]) / totalPointCount)
      : 128;
...
function makeRawPointLoader(
  ...
  return async (entry: HierarchyNodeEntry, budget: number) => {
    ...
    const fraction = viewportFraction(entry.bounds, sourceExtent);
    const effectiveSample = Math.min(
      view.pointCount,
      Math.ceil(Math.max(1, budget) / Math.max(fraction, MIN_VIEWPORT_FRACTION))
    );
    const step = Math.max(1, Math.ceil(view.pointCount / effectiveSample));
...
> {
  ...
  override setVisibility(visibility: boolean): void {
    const wasVisible = this.layerVisible;
    super.setVisibility(visibility);
    this.layerVisible = visibility;
  ...
  private resumeLoading() {
    if (!this.bootstrapState) {
      ...
      void this.refreshForView("moveend");
    }
  }
  ...
  private async refreshBootstrapView(
    ...
    for (const [index, entry] of selection.selectedNodes.entries()) {
      ...
      logInfo("node points loaded", {
        ...
        budget: nodeResult.budget,
        viewportFraction: roundNumber(nodeResult.viewportFraction, 6),
        effectiveSample: nodeResult.effectiveSample,
        step: nodeResult.step,
        loadedPointCount: nodeResult.points.length,
