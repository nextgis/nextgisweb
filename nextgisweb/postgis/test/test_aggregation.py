...
def aggregate(app, layer_id, items, *, filter=None, status=200):
    body = {"items": items}
    if filter is not None:
        body["filter"] = filter
    return app.post(
        f"/api/resource/{layer_id}/feature/aggregate",
        json=body,
        status=status,
    ).json


def test_min_max_integer(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "min_max", "field": "age"}]
    )
    assert result["items"] == [{"type": "min_max", "min": 25, "max": 35}]


def test_min_max_real(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "min_max", "field": "score"}]
    )
    assert result["items"] == [{"type": "min_max", "min": 6.5, "max": 9.0}]


def test_min_max_string(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "min_max", "field": "name"}]
    )
    assert result["items"] == [{"type": "min_max", "min": "Alice", "max": "Eve"}]


def test_min_max_date(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "min_max", "field": "birth_date"}]
    )
    assert result["items"] == [{"type": "min_max", "min": "1988-12-01", "max": "1998-05-15"}]


def test_min_max_multiple_batched(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
...
def test_min_max_field_by_id(ngw_webtest_app, postgis_filter_layer_id):
    info = ngw_webtest_app.get(f"/api/resource/{postgis_filter_layer_id}").json
    age_id = next(f["id"] for f in info["feature_layer"]["fields"] if f["keyname"] == "age")
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "min_max", "field": age_id}]
    )
    assert result["items"] == [{"type": "min_max", "min": 25, "max": 35}]


def test_min_max_with_filter(ngw_webtest_app, postgis_filter_layer_id):
...
def test_unique_values_basic(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app, postgis_filter_layer_id, [{"type": "unique_values", "field": "city"}]
    )
    (item,) = result["items"]
...
def test_unique_values_order_value_asc(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_order_value_desc(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_order_count_desc(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_order_count_asc(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_limit_and_overflow(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_include_counts_false(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_integer_field(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
...
def test_unique_values_with_filter(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
        [{"type": "unique_values", "field": "city", "order": "value_asc"}],
...
def test_mixed_aggregations(ngw_webtest_app, postgis_filter_layer_id):
    result = aggregate(
        ngw_webtest_app,
        postgis_filter_layer_id,
        [
