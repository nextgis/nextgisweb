...
@inject()
def require_storage_enabled(*, core: CoreComponent = inject.arg()):
...
@inject()
def storage_estimate_get(
    request: Request,
    *,
    core: CoreComponent = inject.arg(),
) -> StorageEstimateGetResponse:
...
@inject()
def storage(request: Request, *, core: CoreComponent = inject.arg()) -> StorageResponse:
    """Read storage usage
...
class csetting:
    ...
    @inject()
    def getter(self, *, core: CoreComponent = inject.arg()) -> SValue:
        try:
    ...
    @inject()
    def setter(self, value: SValue | None, *, core: CoreComponent = inject.arg()):
