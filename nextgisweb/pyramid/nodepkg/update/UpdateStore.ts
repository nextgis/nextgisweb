...
export class UpdateStore {
  ...
  private request() {
    ...
    sortBy(Object.entries(ngwConfig.packages), ([n]) => n).forEach(([pn, pv]) =>
      paramsBase.push(`package=${enc(pn)}:${enc(pv)}`)
    );
    ...
    const process = ({ distribution: udist }: UpdateQueryResponse) => {
      ...
      if (status === "not_found" || status === "up_to_date") {
      ...
      } else if (status === "has_update" || status === "has_urgent_update") {
        ...
        this.setState(["has_update", { version, date, critical, notesUrl }]);
      }
    };
