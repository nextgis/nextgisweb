...
def cwebapp(ngw_env):
    ...
    config.add_route("test.session_kv", "/test/session_kv").add_view(
    ...
    with transaction.manager:
        pattern = "%s%%" % prefix
        Session.filter(Session.store.any(SessionStore.key.like(pattern))).delete(
...
def session_headers(ngw_env):
    def _wrap(session_id):
        return dict(cookie="%s=%s" % (ngw_env.pyramid.options["session.cookie.name"], session_id))
