    ...
    def load_file(self, filename, env_arg=None, *, cog=False):
        ...
        try:
            ...
            if src_osr.IsLocal() and (code := src_osr.GetAuthorityCode(None)) is not None:
                # The coordinate system may be interpreted as 'local' when the definitions
                # from EPSG code and GeoTIFF keys are inconsistent. Starting with GDAL 3.5,
                # the GTIFF_SRS_SOURCE configuration option can be used to control this behavior.
