...
def permission(resource, request) -> JSONType:
    ...
    # In some cases it is convenient to pass empty string instead of user's
    # identifier, that's why it so tangled.
...
def quota(request) -> JSONType:
    ...
    result = dict(limit=quota_limit, resource_cls=quota_resource_cls, count=count)
