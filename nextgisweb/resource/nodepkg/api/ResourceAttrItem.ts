...
type ObjFromKeys<K extends `${string}.${string}` & keyof ResourceAttrTypes> = {
  [P in ClsName<K>]: {
    [N in CompName<Extract<K, `${P}.${string}`>>]: ResourceAttrTypes[Extract<
      K,
      `${P}.${N}`
    >];
  };
...
export class ResourceAttrItem<A extends Attributes = Attributes> {
  ...
  update(attrs: Attributes, values: unknown[]): void {
    ...
    for (let i = 0; i < prepared.length; i++) {
      ...
      const idx = this.attrs.findIndex((existing) => sameSpec(existing, spec));
  ...
  async fetch<B extends [...Attributes]>(
    ...
    if (missing.length) {
      const rowValues = await resourceAttrBatcher.load(this.id, missing, opt);
