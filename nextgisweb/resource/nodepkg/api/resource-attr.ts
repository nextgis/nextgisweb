...
export async function resourceAttrItems<A extends [...Attributes]>({
  ...
  const useCache = (
    cacheProp ? (cacheProp instanceof LoaderCache ? cacheProp : cache) : false
    ...
    for (const [id, values] of items) {
      ...
      if (useCache) {
        ...
        if (item) {
          ...
          const newItem = new ResourceAttrItem(id, preparedAttrs, normalized);
          useCache.promiseFor(String(id), async () => newItem);
          resItems.push(newItem);
        }
      } else {
        resItems.push(new ResourceAttrItem(id, preparedAttrs, normalized));
      }
    }
