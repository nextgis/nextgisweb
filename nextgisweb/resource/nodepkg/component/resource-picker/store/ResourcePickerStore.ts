...
> {
  ...
  constructor({
    ...
    if (attributes) {
      this.attributes = [
        ...this.attributes,
        ...attributes,
      ] as typeof ResourcePickerDefaultAttrs;
    }
  ...
  @actionHandler
  async createNewGroup(name: string): Promise<ResourcePickerAttr | undefined> {
  ...
  private _checkConditions({
    ...
    if (requireClass.length) {
      checks.push(() =>
        this.getResourceClasses([cls]).some((cls) => requireClass.includes(cls))
      );
    }
