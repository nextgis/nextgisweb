...
import type { EditorWidget as IEditorWidget } from "../type";
            ...
            <Area pad>
                <Lot label={gettext("Display name")}>
                    <InputValue
                        ...
                        placeholder={
                            store.sdnDynamic || store.sdnBase || undefined
                        }
                    />
                </Lot>
                ...
                {store.composite.resourceId !== 0 && (
                    <Lot label={gettext("Parent")}>
                        <ResourceSelect
                            value={
                                store.parent !== null ? store.parent : undefined
                            }
                            onChange={(v) =>
                                store.update({
                ...
                <Lot label={gettext("Owner")}>
                    <PrincipalSelect
                        ...
                        onChange={(v) =>
                            store.update({ ownerUser: v as number })
                        }
                        allowClear={false}
