...
class MetadataScope(Scope):
    ...
    A typical example of resource metadata is a description in free form. This
    description doesn't affect anything it's change doesn't change data
    structure or anything else. As every resource has description this set of
    permissions is included for all resources at Resource class level."""
...
class DataStructureScope(Scope):
    """Set of permissions for data structure

    For example fields structure of vector layer, its change might lead to
...
class ConnectionScope(Scope):
    """Set of permissions for external connection parameters

    In some cases we need to store access parameters to external resources.
    These parameters may be sensitive, logins and passwords to access remote DB
    for example."""
...
class ServiceScope(Scope):
    """Set of permissions for services such as WMS or WFS

    This is needed to separate permissions for service parameters and its actual
    usage. Though if service is using other resources inside, we need to their
