...
def test_disable_resources(ngw_env, ngw_webtest_app, ngw_resource_group):
    ...
    with ngw_env.resource.options.override(
        {
            "disabled_cls": ["resource_group"],
        }
    ):
...
def test_resource_search(resource, ngw_webtest_app):
    ...
    resp = ngw_webtest_app.get(
        api_url,
        dict(display_name="Test Юникод Symbols ~%"),
        status=200,
    )
    ...
    resp = ngw_webtest_app.get(
        api_url,
        dict(display_name="Test Юникод Symbols ~%", keyname="other"),
        status=200,
    )
    ...
    resp = ngw_webtest_app.get(
        api_url,
        dict(display_name__ilike="test юни%"),
        status=200,
    )
    assert len(resp.json) == 1
...
def test_quota(ngw_resource_group, resource_stat, override, ngw_webtest_app):
    ...
    with override(resource_by_cls=dict(resource_group=rg_count)):
        check_quota(dict(resource_group=0), 200)
        check_quota(
            dict(resource_group=1), 402, dict(cls="resource_group", required=1, available=0)
        )
