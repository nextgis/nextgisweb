...
def test_resource_group(drv, ngw_httptest_app, ngw_resource_group):
    url_create = (
        "NGW:" + ngw_httptest_app.base_url + "/resource/{}/gdal-test".format(ngw_resource_group)
    )

    ds = drv.Create(
        url_create, 0, 0, 0, gdal.GDT_Unknown, options=["DESCRIPTION=test resource group"]
    )
    ...
    url_delete = (
        "NGW:" + ngw_httptest_app.base_url + "/resource/{}".format(ds.GetMetadataItem("id", ""))
    )
