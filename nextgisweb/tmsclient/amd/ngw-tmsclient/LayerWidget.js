    ...
    return declare(
        ...
        ],
        {
            ...
            _setLayerNameVision: function (visible) {
                this.wLayerName.domNode.parentElement.parentElement.style.display =
                    visible ? "" : "none";
            },
            ...
            postCreate: function () {
                ...
                this.wConnection.on(
                    ...
                    function (event) {
                        ...
                        api.route("resource.item", value)
                            .get()
                            .then(
                                function (resource) {
                                    var connection =
                                        resource.tmsclient_connection;
                                    this._setLayerNameVision(
                                        connection.url_template.includes(
                                            "{layer}"
                                        )
                                    );
                                    if (
                ...
                this.btnChooseLayer.on(
                    "click",
                    function () {
                        var layersDialog = new LayersDialog({
                            store: this._store,
                        });
                        layersDialog.pick().then(
                            function (data) {
