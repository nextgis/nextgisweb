    ...
    return (
      <Area pad cols={["1fr", "1fr", "1fr", "6em"]}>
        <Lot row label={gettext("Type")}>
          <Select
            value={store.capmode.value === null ? "" : store.capmode.value}
            onChange={(val) => {
              store.capmode.value =
                val === "" ? null : (val as typeof store.capmode.value);
            }}
            options={[
...
TmsClientConnectionWidget.activateOn = { create: true, update: true };
