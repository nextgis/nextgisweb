...
export const TmsClientLayerWidget: EditorWidget<TmsClientLayerStore> = observer(
  ({ store }) => {
    ...
    const hasLayer = useMemo(() => {
      return (
        store.layer_name.value || (urlTemplate && urlTemplate.search("{layer}"))
      );
    ...
    useEffect(() => {
      ...
      async function fetchConnectionType(resourceId: number) {
        ...
        try {
          const resource = await route("resource.item", resourceId).get({
            signal,
          });
          if ("tmsclient_connection" in resource) {
            const tmsclientConnection = resource.tmsclient_connection;
    ...
    useEffect(() => {
      if (layers && store.composite.operation === "create") {
        const layer = layers.find((l) => l.layer === store.layer_name.value);
        if (layer) {
...
TmsClientLayerWidget.activateOn = { create: true, update: true };
