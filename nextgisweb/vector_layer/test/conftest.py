...
def vector_layer_filter_geojson():
    return json.loads(json.dumps(FILTER_DATASET))


@pytest.fixture(scope="module")
def vector_layer_filter_dataset(vector_layer_filter_geojson):
    with transaction.manager:
        layer = VectorLayer().persist().from_ogr(json.dumps(vector_layer_filter_geojson))
        DBSession.flush()
    return layer.id
