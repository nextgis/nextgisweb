...
class VLSchema(MetaData):
    ...
    def dml_update(self, *, id, with_geom, fields=None):
        ...
        if with_geom:
            values["geom"] = bp = bindparam("geom")
            dif.append(sql_cast(bp, self.geom_column_type).is_distinct_from(_lc_geom))
    ...
    def dml_restore(self):
        ...
        sht = select(ht.c.fid, ht.c.nid, ht.c.nop, ht.c.geom)
        sht = sht.add_columns(*(literal_column(f.name) for f in ct.fields.values()))
        ...
        return uet.returning(et.c.fid.label("id"))
