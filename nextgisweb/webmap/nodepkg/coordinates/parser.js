...
const _handleHemisphereSignByIndex = (coordinates, index) => {
    ...
    coordinates[`isLonLat${index}`] = isNumberXY(coordinates[index.toLowerCase()], sign);
};
...
const makeCoordinatesPairs = (coordinates) => {
    ...
    if (isLonLatA[0] && isLonLatB[1] &&
        isLonValid(a) && isLatValid(b)) {
        coordinatesPairs.push({lon: a, lat: b});
    }

    if (isLonLatA[1] && isLonLatB[0] &&
        isLonValid(b) && isLatValid(a)) {
...
const handleSpaceSeparator = (input) => {
    ...
    return `${matchBySpaces.slice(0, half).join(' ')}, ${matchBySpaces.slice(half).join(" ")}`;
}
