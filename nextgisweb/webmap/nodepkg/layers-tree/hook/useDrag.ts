...
type AllowDrop = NonNullable<TreeProps["allowDrop"]>;
  ...
  const onDrop = useCallback<OnDrop>(
    (info) => {
      ...
      if (info.dropToGap) {
        ...
        const uiTargetIndex = info.dropPosition === -1 ? uiIndex : uiIndex + 1;
      ...
      if (dragItem.parentId === targetParentId) {
        const fromIndex = siblingsStore.findIndex((n) => n.id === dragItem.id);
