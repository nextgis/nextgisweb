    ...
    (frameState: ViewStateLayerStateExtent) => {
      ...
      if (attributions !== undefined) {
        if (Array.isArray(attributions)) {
          attributions.forEach((item) => visibleAttributions.add(item));
        } else {
  ...
  const update = useCallback(
    async (frameState: ViewStateLayerStateExtent | null) => {
      ...
      const attributions = await Promise.all(
        Array.from(collectSourceAttributions(frameState)).map((attribution) =>
    ...
    modal.info({
      ...
      content: (
        <div className="ol-attribution-modal">
          <div style={{ fontWeight: 600, marginBottom: 8 }}>{tipLabel}</div>
  ...
  return (
    visible && (
      <MapControl {...mapControlProps}>
        {contextHolder}
        <div
          className={classNames(className, "ol-unselectable", "ol-control", {
