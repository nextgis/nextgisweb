...
const SharePanel = observer<PanelPluginWidgetProps>(({ store, display }) => {
  ...
  const updateEmbedCode = useCallback(() => {
    const iframeSrc = getPermalink({
      ...
      visibleItems: treeStore.visibleLayers,
      urlWithoutParams:
        ngwConfig.applicationUrl + routeURL("webmap.display.tiny", webmapId),
      additionalParams: {
    ...
    <PanelContainer title={store.title} close={store.close}>
      ...
      <PanelSection title={gettext("Embed code for your site")} flex>
        ...
        <div className="input-group">
          ...
          />
          <span className="checkbox__label">{gettext("Link to main map")}</span>
        </div>
        <div className="input-group">
          <Switch
            size="small"
          ...
          />
          <span className="checkbox__label">{gettext("Generate events")}</span>
        </div>
        <div className="input-group  column">
        ...
        <form action={previewUrl} method="POST" target="_blank">
          <input type="hidden" name="iframe" value={encodeURI(embedCode)} />
          <Space.Compact>
            <CopyToClipboardButton
