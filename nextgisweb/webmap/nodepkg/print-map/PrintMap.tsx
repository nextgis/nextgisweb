  ...
  ({
    ...
    return (
      <RndComp
        ...
        movable={false}
      >
        <div className="print-olmap" style={{ width: "100%", height: "100%" }}>
          <PrintMapPreview display={display} printMapStore={printMapStore} />
        </div>
      </RndComp>
    );
  }
    ...
    return (
      <LegendPrintMap
        ...
        onChange={(rndCoords: LegendRndCoords) => {
          printMapStore.layout.updateCoordinates("legendCoords", rndCoords);
        }}
      />
    );
  }
...
export const PrintMap = observer<PrintMapProps>(
  ({ display, printMapStore }) => {
    ...
    }, [title, width, height, margin, legend, legendColumns, printMapStore]);
        ...
        <div className="print-map-export-wrapper">
          <div className="print-map-page">
            <div id="printMap" className="print-map">
              <LegendComp display={display} printMapStore={printMapStore} />
              <TitleComp printMapStore={printMapStore} />
              <MapComp display={display} printMapStore={printMapStore} />
            </div>
          </div>
        </div>
      </div>
