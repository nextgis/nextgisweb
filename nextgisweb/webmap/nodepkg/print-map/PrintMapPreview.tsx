...
import { useDebounceCallback } from "@nextgisweb/pyramid/hook";
...
export const PrintMapPreview = observer(
    ({
        ...
        onCenterChange,
        onScaleChange,
    }: PrintMapPreviewProps) => {
        const [mapStore] = useState(() => {
            const projection = "EPSG:3857";
            const resolutions = buildResolutions(PRINT_SCALES, projection);
            const view = new View({
                projection,
                resolutions,
        ...
        const { arrow, scale, width, height, margin, scaleLine, scaleValue } =
            settings;
        ...
        const debouncedOnScaleChange = useDebounceCallback(onChangeScale, 200);
        ...
        useEffect(() => {
            if (scale) {
                if (!PRINT_SCALES.includes(scale)) {
                    replaceViewResolutions(mapStore, [scale, ...PRINT_SCALES]);
                }

                mapStore.olView.setResolution(scaleToResolution(scale));
            }
        }, [mapStore, mapStore.olView, scale]);

        useEffect(() => {
            if (mapStore.scaleForResolution !== undefined) {
                debouncedOnScaleChange(mapStore.scaleForResolution);
            }
        }, [mapStore.scaleForResolution, debouncedOnScaleChange]);
