...
export class TreeStore {
  ...
  async updateResourceLegendSymbols(resources: number[]) {
    if (resources.length) {
      ...
      try {
        newResources.forEach((id) => this._loadingResourceSymbols.add(id));
        const legends = await route("render.resource_legend_symbols").get({
        ...
        console.error("Error fetching render.resource_legend_symbols:", err);
      } finally {
        newResources.forEach((id) => this._loadingResourceSymbols.delete(id));
      }
    }
  }
