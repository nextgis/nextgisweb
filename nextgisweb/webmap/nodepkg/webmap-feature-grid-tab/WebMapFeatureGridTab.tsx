...
export const WebMapFeatureGridTab = observer(
  ({ topic, plugin, item }: WebMapFeatureGridTabProps) => {
    const { layerId, filter, lastSyncedAt } = item;
      ...
      const store = new FeatureGridStore({
        ...
        onSelect: (newVal) => {
          ...
          if (fid !== undefined) {
            ...
            display.highlighter.unhighlight((e) => e.layerId === layerId);
          }
        },
        actions: [
          {
                ...
                route("feature_layer.feature.item", {
                  ...
                  .then((feature) => {
                    if (feature.geom !== null) {
                      ...
                      showMessage(
                        "warning",
                        gettext("Selected feature has no geometry")
                      );
                    }
                  });
              }
    ...
    }, [lastSyncedAt, makeSignal]);

    useEffect(() => {
      if (!store) return;
      const featureUpdatedEvent = ({ resourceId }: { resourceId: number }) => {
