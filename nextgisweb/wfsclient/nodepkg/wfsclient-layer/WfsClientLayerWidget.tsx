...
const geometries: Record<string, FeatureLayerGeometryType> = {
  "Point": "POINT",
...
async function inspectLayerFetch(
  ...
  const result: Record<string, FeatureLayerGeometryType> = {};
...
export const WfsClientLayerWidget: EditorWidget<WfsClientLayerStore> = observer(
  ({ store }) => {
    ...
    const autocompleteProps = useCallback(
      (
        ...
        loading:
          status === "loading" && focusedField === field ? true : undefined,
        style: { width: "100%" },
        options,
      }),
      [focusedField, store]
    ...
    const updateGeometryType = (value: FeatureLayerGeometryType) => {
    ...
    return (
      <Area pad cols={["1fr", "1fr"]}>
        ...
        />
        <Lot error={store.columnGeom.error} label={gettext("Geometry column")}>
          <AutoCompleteInput
            {...autocompleteProps("columnGeom", layerInfo, columnGeomOptions)}
            onChange={(value) => {
