...
class ComparisonOperator(Operator):
    ...
    def createStatement(self, datasource):
        xslt = etree.parse(os.path.dirname(os.path.abspath(__file__)) +
                           "/../../../../resources/filterencoding/comparison_operators.xsl")
        ...
        if hasattr(datasource, 'hstore'):
            result = transform(self.node, datasource="'" + datasource.type + "'", operationType="'" + str(self.node.xpath('local-name()'))
                               + "'", hstore="'" + str(datasource.hstore).lower() + "'", hstoreAttribute="'" + datasource.hstoreAttribute + "'")
        else:
            result = transform(self.node, datasource="'" + datasource.type +
                               "'", operationType="'" + str(self.node.xpath('local-name()')) + "'")
