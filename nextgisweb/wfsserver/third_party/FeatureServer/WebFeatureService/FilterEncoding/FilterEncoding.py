...
class FilterEncoding (object):
    ...
    def __init__(self, xml):
        ...
        # add global namespaces - duplicate namespaces will be removed by
        # parser.
        nsFilter = '<Filter'
    ...
    def getFilterInstance(self, node):
        try:
            ...
            sys.path.append(
                os.path.dirname(os.path.abspath(__file__)) + "/ComparisonOperators")
            sys.path.append(
                os.path.dirname(os.path.abspath(__file__)) + "/LogicalOperators")
            sys.path.append(
                os.path.dirname(os.path.abspath(__file__)) + "/ObjectIdentifiers")
            sys.path.append(
                os.path.dirname(os.path.abspath(__file__)) + "/SpatialOperators")
            operator_module = __import__(
                str(node.xpath('local-name()')), globals(), locals())
        except ImportError:
            raise Exception("Could not find filter for %s" %
